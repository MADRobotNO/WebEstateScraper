...
class Database:
    ...
    def __init__(self):
        ...
        self.con = MySQLdb.connect("xxx", "xxx", "xxx", "xxx")
        self.cursor = self.con.cursor()

    def connect_to_db(self):
        self.cursor.execute("CREATE TABLE IF NOT EXISTS estates_Hamar (id INTEGER PRIMARY KEY NOT NULL AUTO_INCREMENT, "
    ...
    def insert_to_db(self, location, type_estate, price, size, price_per_kvm, bedroom, date, description):
        self.cursor.execute("INSERT INTO estates_Hamar VALUES (NULL, %s,%s,%s,%s,%s,%s,%s,%s)"
                            , (location, type_estate, price, size, price_per_kvm, bedroom, date, description))
        self.con.commit()

    def view_db(self):
        self.cursor.execute("SELECT * FROM estates_Hamar")
        rows = self.cursor.fetchall()
        return rows

    def search_in_db(self, location="", type_estate="", price="", size="", price_per_kvm="", bedroom="",
                     date='0000-00-00'):
        self.cursor.execute("SELECT * FROM estates_Hamar WHERE location=%s OR type_estate=s% OR price=%s OR "
    ...
    def delete_from_db(self, id="", location="", type_estate="", price="", size="", price_per_kvm="", bedroom="",
                       date='0000-00-00'):
        self.cursor.execute("DELETE FROM estates_Hamar WHERE id=%s OR location=%s OR type_estate=%s OR price=%s OR "
                            "size=%s OR price_per_kvm=%s OR bedroom=%s OR date=%s",
                            (id, location, type_estate, price, size, price_per_kvm, bedroom, date))
        self.con.commit()

    def check_if_exists(self, description):
        self.cursor.execute("SELECT * FROM estates_Hamar where description=%s", (description,))
    ...
    def show_columns(self):
        self.cursor.execute("SHOW FIELDS FROM estates_Hamar")
        rows = self.cursor.fetchall()
    ...
    def get_prices(self):
        self.cursor.execute("SELECT price FROM estates_Hamar")
        rows = self.cursor.fetchall()
        return rows

    def get_prices_per_kvm(self):
        self.cursor.execute("SELECT price_per_kvm FROM estates_Hamar ")
        rows = self.cursor.fetchall()
        return rows
